...
- **template_9**: НЕ перекладений — лише de, fr, it, nl мають реальний
  переклад (~40KB кожен, побайтово ідентичний з тим, що було в template_8
  до фіксу — скопійовано з одного шаблону в інший). Решта 18 мов + en —
  локаль-заглушки (370 байт, контент лишається англійською). Переклад
  геосторінок явно призупинений користувачем (підтверджено 2026-09-01) —
  не чіпати без окремого запиту.
