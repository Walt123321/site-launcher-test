...
- **template_8**: повністю перекладений (2026-09-01) — усі 22 мовні
  `lang.php` + en-stub, кожен з повним реальним перекладом 522 рядків
  (не 291 як у T7/T10 — цей шаблон значно більший). de/fr/it/nl спочатку
  містили лише 391/522 рядків (бракували цілі секції: about/product/
  offer/contacts/faq/privacy/terms/risk/sign сторінки, nav, footer,
  badges, trust-блоки) — довершені до 522/522 перед тим як почались нові
  18 мов. nb і no — однаковий текст (обидва bokmål).
- **template_9**: повністю перекладений (2026-09-01, пізніше того ж дня) —
  усі 22 мовні `lang.php` (bg, cs, da, de, el, es, fi, fr, hr, hu, it, ja,
  ms, nb, nl, no, pl, pt, ro, sk, sv, tr) з реальним перекладом 286 рядків
  + вже наявна коректна en-заглушка. de/fr/it/nl раніше мали побайтово
  ідентичний з template_8 контент — виявилось, що це був застарілий
  контент з іншою структурою (4 відгуки з назвами країн замість 5 відгуків
  з іменами людей), непридатний для цього шаблону — усі 22 мови
  перероблено з нуля з чистого 286-рядкового екстракту кореневого
  lang.php. nb і no — однаковий текст (обидва bokmål). Переклад мовних
  рядків завершено; PHP-шаблони геосторінок (index.php тощо) досі містять
  застарілі undefined-variable баги зі старого скаффолду (напр.
  `$form_name_placeholder` замість реального `$form_fname_placeholder`) —
  не виправлено цією сесією, окреме завдання.
